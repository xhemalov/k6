...
const successSaveMovies = new Counter("successSaveMovies");
const successSaveReviews = new Counter("successSaveReviews");
let resCheck = {};

const saveMovies = function (
  config = fail(`missing config.`),
  movie = fail(`missing config.`)
) {
  group(`Save movie`, function () {
    movie["id"] = randomIntBetween(1, 1000000);
    let res = http.post(
      `${config.host}/${config.index}/_doc/${uuidv4()}`,
...
const saveReview = function (
  config = fail(`missing config.`),
  review = fail(`missing config.`)
) {
  group(`Save review`, function () {
    review["id"] = randomIntBetween(1, 1000000);
    let res = http.post(
      `${config.host}/${config.reviews_index}/_doc/${uuidv4()}`,
